    ...
    public partial class AESEncrypt : Form
    {
        ...
        private void CryptButton_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception)
            {
                ...
                if (selLanguage.Contains("zh"))
                {
                    MessageBox.Show("发现错误了！" + " 执行失败-密码错误？", "K2Lin Crypto （；´д｀）ゞ", MessageBoxButtons.OK);
                }
                else
                {
                    MessageBox.Show("Error found!" + " Failed to crypt - wrong password?", "K2Lin Crypto （；´д｀）ゞ", MessageBoxButtons.OK);
                }
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (selLanguage.Contains("zh"))
            {
                MessageBox.Show("已生成强密钥并复制到剪贴板！", "K2Lin Crypto (｡･∀･)ﾉﾞ", MessageBoxButtons.OK);
            }
            else
            {
                MessageBox.Show("A strong key has been generated and copied to the clipboard!", "K2Lin Crypto (｡･∀･)ﾉﾞ", MessageBoxButtons.OK);
            }
        }
    }
}
